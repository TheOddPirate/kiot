...
public slots:
    ...
    {
        //qCDebug(plugin_logger_activewindow) << "UpdateAttributes called with:" << attributes;
...
private:
    ...
    bool registerKWinScript()
    {
        ...
        //qCDebug(plugin_logger_activewindow) << "Script loaded at" << scriptObjectPath;
        ...
        if (runReply.type() == QDBusMessage::ErrorMessage) {
            //qCWarning(plugin_logger_activewindow) << "Failed to run script" << runReply.errorMessage();
            return false;
        }
...
{
    return QString(PLUGIN_DESCRIPTION).replace("\"", "") + QString(" ").replace("\"", "") + QString(PLUGIN_DOMAIN).replace("\"", "");
}
