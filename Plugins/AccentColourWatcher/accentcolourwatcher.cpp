...
Q_DECLARE_LOGGING_CATEGORY(plugin_logger_accentcolour)
AccentColourWatcher::AccentColourWatcher(QObject *parent)
    : QObject(parent)
    , m_sensor(nullptr)
{
...
void AccentColourWatcher::updateAccentColor(const KConfigGroup &config)
{
...
QString AccentColourWatcher::rgbToHex(const QString &rgb)
{
...
void AccentColourWatcher::setRgbAttributes(QVariantMap &attributes, const QString &rgb, const QString &prefix)
{
    QStringList parts = rgb.split(",");
    if (parts.size() == 3) {
        bool ok;
